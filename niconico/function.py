...
class AssTranslatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ASS 弹幕翻译助手 (v2.0 修正版)")
    ...
    def _get_dialogue_parts(self, line):
        """
        稳健地解析Dialogue行，返回前缀、样式和纯文本。
        Returns: (prefix, style, text) or (None, None, None)
        """
        line = line.strip()
    ...
    def run_check_ass_format(self):
        ...
        try:
            ...
            self.log(f"找到 '[Events]:' 行在第 {format_line_index + 2} 行。开始从下一行扫描...")

            # 2. 从 "[Events]:" 的下一行开始逐行扫描
            for i in range(format_line_index + 2, len(lines)):
                ...
                # 3. 核心检查：每一行都必须以 "Dialogue:" 开头
                if not line.startswith("Dialogue:"):
                    self.log(f"第 {line_num} 行错误: 该行应以 'Dialogue:' 开头，但实际内容是: '{line}'")
                    invalid_lines.append(line_num)
                    continue # 一旦不是Dialogue开头，就不再进行后续检查，直接跳到下一行

                # 4. 如果是以 "Dialogue:" 开头，再进行详细的内部格式检查
                line_errors = []
                parts = line.split(',', 9)
                if len(parts) != 10:
                    line_errors.append("逗号数量不正确 (应为9个)")
                
                # 即使逗号数量不对，也尝试检查时间格式，提供更多信息
            ...
            if not invalid_lines:
                self.log("检查完成。从 'Format:' 行之后的所有行均满足格式要求。")
    ...
    def run_scan_existing_translations(self):
        ...
        try:
            self.log("步骤2: 读取翻译数据库...")
            trans_dict = {}
            with open(csv_file, 'r', encoding='utf-8') as f:
                reader = csv.reader(f)
                for row in reader:
                    if len(row) >= 2:
                        trans_dict[row[0]] = row[1]
            self.log(f"成功加载 {len(trans_dict)} 条翻译记录。")
    ...
    def run_extract_needed_translations(self):
        ...
        try:
            with open(already_txt_file, 'r', encoding='utf-8') as f:
                already_indices = set(f.read().strip().split(','))
            ...
            with open(jp_ass_file, 'r', encoding='utf-8') as f:
                for i, line in enumerate(f):
                    ...
                    if line_num not in already_indices:
                        ...
                        if text is not None: # 如果是有效的Dialogue行
    ...
    def run_fill_in_translations(self):
        ...
        try:
            with open(already_txt_file, 'r', encoding='utf-8') as f:
                already_indices_str = f.read().strip().split(',')
                already_indices = set(int(x) for x in already_indices_str if x)
            self.log(f"读取到 {len(already_indices)} 个已翻译行号。")
