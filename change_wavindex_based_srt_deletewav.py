...
def rename_wav_files_by_srt_text(srt_file_path, wav_folder_path, delete_unmatched_wavs=False):
    ...
    新增功能：如果 delete_unmatched_wavs 为 True，则删除在SRT中找不到匹配项的WAV文件。
    """
    print(f"---- 开始处理 ----")
    print(f"正在读取SRT文件: {srt_file_path}")
    if delete_unmatched_wavs:
        print("警告：删除不匹配WAV文件的功能已启用！")
    else:
        print("提示：删除不匹配WAV文件的功能已禁用。")
    ...
    deleted_count = 0 # 新增：用于计被删除的文件数量

    original_wav_files = os.listdir(wav_folder_path) # 先获取文件列表

    for filename in original_wav_files:
        ...
        if processed_wav_text in srt_text_to_index_map:
            ...
            if full_wav_path != new_full_path:
                ...
                # 文件名已经符合预期，无需重命名
                print(f"已正确命名： '{filename}' 具有正确的序号前缀 '{matched_srt_index_padded}'。")
            ...
            if delete_unmatched_wavs:
                ...
                try:
                    ...
                    print(f"删除：未在SRT中找到匹配项，已删除文件 '{filename}'。")
                    deleted_count += 1
                except OSError as e:
                    print(f"错误：无法删除文件 '{full_wav_path}': {e}")
                    skipped_count += 1
            else:
                # 如果未启用删除功能，则只报告
                print(f"警告：未在SRT中找到与 '{filename}' (处理后文本: '{processed_wav_text}') 匹配的文本。文件未被操作。")
    ...
    if delete_unmatched_wavs:
        print(f"因未在SRT中匹配而被删除的文件数量： {deleted_count}")
    else:
        print(f"未在SRT中找到匹配的文件数量（未删除）： {not_found_in_srt_count}")
...
if __name__ == "__main__":
    ...
    # !!! 重要：设置此项来控制是否删除不匹配的WAV文件 !!!
    # 设置为 True  -> 会删除在SRT中找不到对应文本的WAV文件。
    # 设置为 False -> 不会删除任何文件，仅报告不匹配的文件（与原脚本行为一致）。
